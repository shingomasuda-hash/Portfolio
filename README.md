...
書棚そのものを部屋の中の家具として組んでいます。床・壁・下段のキャビネット、
天板の上には平積みの本・器・植物、棚の端には書類ホルダー。
背表紙にカーソルを置く（または右の目次を指す）と、その本が一冊だけ手前へ抜け出します。
...
「どの業界の・どういった会社が・どんな数字を出したか」は
[`src/data/outcomes.ts`](src/data/outcomes.ts) の1ファイルに集約しています。
ここに数字を書き足すと、次のすべてに自動で反映されます。
...
```ts
figures: [
  { value: "132%", label: "月商", period: "支援12ヶ月", basis: "前年同月比" },
],
```

`figures` が空の案件は数字が出ず、`results` の定性的な変化だけが表示されます。
`sector`（業界）・`area`・`scale`・`profile`（どういった会社か）も同じファイルです。

> 現状、数字が入っているのは FOOD BRANDING の2件だけです。
> 数字は実際の取引先の成果なので、こちらでは作っていません。分かるものから埋めてください。

## 画面に出す文章

3Dだけでは「何をしたのか」が伝わらないので、左の段組みが場面ごとに中身を説明します。

| 場面 | 左段に出るもの |
| --- | --- |
| INTRO | 担当領域（SCOPE）の一覧 |
| CHALLENGE | 課題の本文 |
| WHAT WE DID | 施策を番号付きで、一行の日本語解説つき |
| OUTPUT | つくったもの。ラベル・種類・`outputs[].description` をそのまま |
| RESULT | 出た数字（`outcomes.ts`）と、定性的な変化（`results[]`） |

INTRO には「どんな会社か（業界・エリア・規模・会社像）」も出ます。
上部の **WORKS** から、10件を業界・会社像・つくったもの・成果の表で一覧できます。

施策の日本語解説は [`src/data/glossary.ts`](src/data/glossary.ts) にまとめてあります。
`approach[]` の英語キーワードに対する一行訳なので、言い回しは自由に書き換えてください。
未登録の語はキーワードだけが表示されます。

OUTPUT の画像が未配置のときは、その制作物の名前（BRAND / STORE / MENU …）と種類が入った
色付きのプレートが立ち上がります。写真が無くても「何をつくったか」は読めます。
